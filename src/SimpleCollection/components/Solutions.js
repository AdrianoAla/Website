...
export default function Solutions({ block, profile, website }) {
  ...
  return (
    <div className={`${block.theme} py-24 bg-[var(--primary)] sm:py-32`}>
...
const FeatureCard = ({feature, profile}) => {
  return (
    <div key={feature.name} className="relative p-8 transition-all ease-in-out border rounded-md hover:border-[var(--on\_primary)] hover:scale-105 bg-[var(--primary)]">
      <dt className="flex flex-row justify-between text-base font-semibold leading-7 text-gray-900">
        <div className="flex items-center justify-center w-10 h-10 mb-8 ">
          <Image
              ...
              className="w-10 h-10 p-2 bg-[var(--secondary)] 2-md rounded-lg"
            />
        </div>
        {feature.link && <div className="flex items-center justify-center w-10 h-10 mb-8">
          <ArrowUpRightIcon className="w-6 h-6 text-[var(--on\_primary)" aria-hidden="true" />
        </div>}
      </dt>
